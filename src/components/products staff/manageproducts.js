...
const Manageproducts = (props) => {
    ...
    const updateFlag = async (item) => {
        if (!select) {
            ...
            if (item.flag == 1) {
                let res = await updateFlagInProject(item.id, +select, 0)
                if (res && +res.EC === 0) {
                    let abc = await createNotification(item.id, item.order, "nhân viên chuyển trạng tái sang bình thường", `${user.account.username}-${user.account.phone}`, item.createdBy, 0, 1, select)
                    if (abc && +abc.EC === 0) {
                        await fetchProjectUser(select)
                        if (data) {
                            await HandleSearchData(data)
            ...
            if (item.flag == 0) {
                let res = await updateFlagInProject(item.id, +select, 1)
                if (res && +res.EC === 0) {
                    let abc = await createNotification(item.id, item.order, "nhân viên chuyển trạng tái sang gấp", `${user.account.username}-${user.account.phone}`, item.createdBy, 0, 1, select)
                    if (abc && +abc.EC === 0) {
                        await fetchProjectUser(select)
                        if (data) {
                            await HandleSearchData(data)
    ...
    const fetchProjectUser = async (item) => {
        ...
        if (!item) {
            ...
            let res = await getProjectWithPaginationWithEmployer(currentPage, currentLimit, item)
            if (res && +res.EC === 0) {
                setTotalPage(+res.DT.totalPage)
                if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
                    setCurrentPage(+res.DT.totalPage)
                    await getProjectWithPaginationWithEmployer(+res.DT.totalPage, currentLimit, +item
                    )
                    if (user?.account?.groupName === "Customer" || user?.account?.groupName === "Staff" && user.account.Position) {
                        await getALlListNotification(+user.account.shippingunit_id, user.account.phone, user.account.Position)
                    }
                                        ...
                                        <div className='table-wrapper-employer my-5'>
                                            ...
                                            <div className='container'>
                                                ...
                                                <div style={{ overflow: "auto" }}>
                                                    ...
                                                    <table className="table table-bordered table-body-employer">
                                                        <thead>
                                                            <tr className='table-secondary'>
                                                                <th scope="col">
                                                                    {t('Manage-employer.Body.Tittle-Three')}
