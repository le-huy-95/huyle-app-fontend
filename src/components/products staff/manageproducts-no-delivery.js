...
const ManageproductsNoDelivery = (props) => {
    ...
    const HandleSearchData = debounce(async (value) => {
        if (!select) {
            ...
            if (data) {
                ...
                if (res && +res.EC === 0) {
                    let results = res.DT.filter(item => item.statusdelivery_id === 0)
    ...
    const fetchProjectUser = async (item) => {
        if (!item) {
            let res = await getProjectWithPaginationWithALlStatusDelivery(currentPage, currentLimit, +user.account.shippingunit_id, 0
            )
            if (res && +res.EC === 0) {
                setTotalPage(+res.DT.totalPage)
                if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
                    setCurrentPage(+res.DT.totalPage)
                    await getProjectWithPaginationWithALlStatusDelivery(+res.DT.totalPage, currentLimit, +user.account.shippingunit_id, 0
                    )
                    if (user?.account?.groupName === "Customer" || user?.account?.groupName === "Staff" && user.account.Position) {
                        await getALlListNotification(+user.account.shippingunit_id, user.account.phone, user.account.Position)
                    }
            ...
            let res = await getProjectWithPaginationWithALlStatusDelivery(currentPage, currentLimit, +item, 0)
            if (res && +res.EC === 0) {
                setTotalPage(+res.DT.totalPage)
                if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
                    setCurrentPage(+res.DT.totalPage)
                    await getProjectWithPaginationWithALlStatusDelivery(+res.DT.totalPage, currentLimit, +item, 0)
                    if (user?.account?.groupName === "Customer" || user?.account?.groupName === "Staff" && user.account.Position) {
                        await getALlListNotification(+user.account.shippingunit_id, user.account.phone, user.account.Position)
                    }
                    if (user?.account?.groupName === "Dev") {
    ...
    const fetchProjectUserAfterRefesh = async () => {
        ...
        let res = await getProjectWithPaginationWithALlStatusDelivery(+currentPagelocalStorage, currentLimit, +user.account.shippingunit_id, 0
        )
        if (res && +res.EC === 0) {
            setTotalPage(+res.DT.totalPage)
            if (res.DT.totalPage > 0 && res.DT.dataProject.length === 0) {
